...
CATEGORIES:
- booking: CONFIRMED hotel/resort/villa reservations that (a) contain an explicit confirmation number, booking reference, or reservation number issued by the property, consortium, or GDS, AND (b) have status confirmed or reconfirmed, AND (c) are sent FROM a hotel, resort, consortium (LHW, Serandipians, TravellerMade, THRS, Heavens Portfolio), or GDS system — NEVER from a @kft.travel address. EXCEPTION: Amadeus itinerary documents whose subject matches SURNAME/FIRSTNAME MR/MRS/MS DDMMMYYYY DESTCODE format (e.g. "DHOOT/RADHIKA MRS 04JUL2026 CHI") are classified as booking even when forwarded from a @kft.travel address. Do NOT use this for options, rate quotes, availability responses, or any email lacking a confirmation number.
...
- ops: Internal team emails, admin, industry events, ILTM, webinars, trade shows. Also use ops for any email sent FROM a @kft.travel address regarding an existing booking (room configuration requests, rooming list updates, special request follow-ups, internal coordination about a trip). EXCEPTION: if the subject matches Amadeus itinerary format — a passenger name in SURNAME/FIRSTNAME MR/MRS/MS format followed by a date (DDMMMYYYY) and a destination code (e.g. "DHOOT/RADHIKA MRS 04JUL2026 CHI") — classify as booking instead, even if sent from a @kft.travel address. These are Amadeus documents forwarded through the KFT inbox.
